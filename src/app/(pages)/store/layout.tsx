...
const StoreLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const { brand, openFilter, sort, rating, offer, price } = useAppSelector(
    (state) => state.filter
  );
  ...
  const priceFilter = `&minPrice=${price[0]}&maxPrice=${price[1]}`;

  useFetchProducts(`${brandFilter}${catFilter}${ratingFilter}${offerFilter}${priceFilter}`);
